package user_api

import (
	"gvb_server/gvb_server/global"
	"gvb_server/gvb_server/models"
	"gvb_server/gvb_server/models/res"
	"gvb_server/gvb_server/plugins/email"
	"gvb_server/gvb_server/utils/jwts"
	"gvb_server/gvb_server/utils/pwd"
	"gvb_server/gvb_server/utils/random"

	"github.com/gin-contrib/sessions"
	"github.com/gin-gonic/gin"
)

type BindEmailRequest struct {
	Email    string  `json:"email" binding:"required,email" msg:"邮箱非法"`
	Code     *string `json:"code"`
	Password string  `json:"password"`
}

func (UserApi) UserBindEmailView(c *gin.Context) {
	_claims, _ := c.Get("claims")
	claims := _claims.(*jwts.CustomClaims)
	//用户绑定邮箱，第一次输入是邮箱，
	//后台会给这个邮箱发验证码，
	var cr BindEmailRequest
	err := c.ShouldBindJSON(&cr)
	if err != nil {
		res.FailWithError(err, &cr, c)
		return
	}
	session := sessions.Default(c)
	if cr.Code == nil {
		//第一次用户输入邮箱，后台给这个邮箱发验证码
		// 生成4为验证码,将生成的验证码存入session
		code := random.Code(4)
		//写入session

		session.Set("valid_code", code)
		err := session.Save()
		if err != nil {
			global.Log.Error(err)
			res.FailWithMessage("验证码写入session失败", c)
			return
		}
		err = email.NewCode().Send(cr.Email, "你的验证码是:"+code)
		if err != nil {
			global.Log.Error(err)
			return
		}
		res.OKWithMessage("验证码已发送到邮箱，请查收", c)
		return
	}

	//第二次用户输入邮箱，验证码，密码，完成绑定
	//校验验证码
	code := session.Get("valid_code")
	if code != *cr.Code {
		res.FailWithMessage("验证码错误", c)
		return
	}
	//修改用户的邮箱
	var user models.UserModel
	err = global.DB.Take(&user, claims.UserID).Error
	if err != nil {
		res.FailWithMessage("用户不存在", c)
		return
	}
	if len(cr.Password) < 6 {
		res.FailWithMessage("密码长度必须大于等于6", c)
		return
	}
	hashPwd := pwd.HashPwd(cr.Password)
	// 第一次的邮箱和第二次的邮箱也要做一致性校验,收验证码和绑定验证码的邮箱也要是一样的
	err = global.DB.Model(&user).Updates(map[string]any{
		"email":    cr.Email,
		"password": hashPwd,
	}).Error
	if err != nil {
		res.FailWithMessage("修改邮箱失败", c)
		return
	}

	//完成绑定
	res.OKWithMessage("绑定邮箱成功", c)
}
